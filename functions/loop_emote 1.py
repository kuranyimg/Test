...
async def loop(highrise: Highrise, user: User, message: str) -> None:
    # Defining the loop_emote method locally so it cann't be accessed from the command handler.
    async def loop_emote(highrise: Highrise, user: User, emote_name: str) -> None:
        emote_id = ""
        for emote in emote_list:
            if emote[0].lower() == emote_name.lower():
                ...
                break
        if emote_id == "":
            await highrise.send_whisper(user.id,f"🚫🔄 @{user.username} To Stop the Loop Just Walk🔄🚫")
            return

            # Stop any existing loop for this user
            await stop_loop(highrise, user)

            # Start the new loop
            await highrise.send_whisper(user.id,f"👯🏻‍♂️🔄 @{user.username} You are in a loop: {emote_name} 👯🏻‍♂️🔄")
            while True:
                print(f"Loop {emote_name} - {user.username}")
                try:
                    await highrise.send_emote(emote_id, user.id)
                except Exception as e:
                    await highrise.send_whisper(user.id,f"🚫🔄{user.username} loop 🤍 🔄🚫")
                ...
                if not user_in_room:
                    await highrise.send_whisper(user.id,f"✅️{user.username} Siga <@RayMG> 🤍 Hastag : #RayMG ✅️")
                    return
        ...
        try:
            ...
            taskgroup.create_task(coro=loop_emote(highrise, user, emote_name), name=user.username)

async def stop_loop(highrise: Highrise, user: User, message: str) -> None:
        taskgroup = highrise.tg
        task_list : list[Task] = list(taskgroup._tasks)
        for task in task_list:
            if task.get_name() == user.username:
