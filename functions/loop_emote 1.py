...
async def loop(self: BaseBot, user: User, message: str) -> None:
    async def loop_emote(self: BaseBot, user: User, emote_name: str) -> None:
        ...
        for emote in emote_list:
            if emote_name.isdigit() and emote[0] == emote_name:
                emote_id = emote[1]
                break
            elif emote[1].lower() == emote_name.lower():
                emote_id = emote[1]
                break

        if emote_id == "":
            await self.highrise.send_whisper(user.id, f"🚫🔄 @{user.username} To Stop the Loop Just Walk or Type 'stop' 🔄🚫")
            return
        ...
        if user_position is None:
            await self.highrise.send_whisper(user.id, f"✅️ @{user.username} Follow <RayMG> 🤍 Hashtag: #RayMG ✅️")
            return
...
async def stop_loop(self: BaseBot, user: User, message: str) -> None:
    ...
    for task in task_list:
        if task.get_name() == user.username:
            task.cancel()
            await self.highrise.send_whisper(user.id, f"✅️ @{user.username} Loop stopped successfully ✅️")
            return
    await self.highrise.send_whisper(user.id, f"✅️ @{user.username} No active loop found ✅️")
    return
