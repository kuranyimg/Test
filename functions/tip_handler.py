...
async def handle_tip_command(self, message, user):
    ...
    if message.lower().startswith("-tipall ") or message.lower().startswith("-tipme "):
        if user.username in ["RayBM", "FallonXOXO", "sh1n1gam1699"]:  # Allowed users
            parts = message.split(" ")
            if len(parts) != 2:
            ...
            if message.lower().startswith("-tipall "):
                ...
                for room_user, pos in room_users.content:
                    for bar in tip:
                        ...
                        await self.highrise.chat(f"User ID: {room_user.id} has been tipped {tip_amount}!")
                        await asyncio.sleep(1)

            elif message.lower().startswith("-tipme "):
                for bar in tip:
                    await self.highrise.tip_user(user.id, bar)
                await self.highrise.chat(f"You have been tipped {tip_amount}.")
